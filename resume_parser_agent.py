...
class SheetsWrapper:
    ...
    def update_detail_from_df(self, df: pd.DataFrame, sheet_name: str = 'detail'):
        ...
        sh.clear()
        # Convert all values to strings to avoid Google Sheets API errors with NaN/datetime/dict
        clean_df = df.fillna('').astype(str)
        sh.update([clean_df.columns.values.tolist()] + clean_df.values.tolist())
...
def fetch_resume_text_from_url(url: str) -> str:
    ...
    try:
        ...
        if 'pdf' in ctype or url.lower().endswith('.pdf'):
            ...
            # Save to cross-platform temp file and parse
            tmp_path = os.path.join(tempfile.gettempdir(), 'temp_resume.docx')
...
DEFAULT_SCORING = {
    ...
    'location': {
        'Hyderabad': 15, 'Bengaluru': 10, 'Bangalore': 10,
        'Pune': 8, 'Chennai': 8, 'Mumbai': 8, 'Delhi': 5
    },
    'profile_tagline': 10,
    'resume_quality': 15,
}
...
def score_candidate(candidate: Dict, scoring_config: Dict = None) -> Dict:
    ...
    resume_pts = min(scoring_config.get('resume_quality', 0), int(word_count / 20))  # 20 words = 1 point
...
scheduler = None  # Initialized lazily when scheduling is needed
...
def schedule_followups_for_candidate(candidate_row: Dict, start_date: Optional[datetime] = None):
    ...
    for idx, (offset_days, message) in enumerate(DEFAULT_FOLLOWUPS, start=1):
        ...
        get_scheduler().add_job(make_job(candidate_email, f"Follow-up #{idx}", message), 'date', run_date=run_date, id=job_id)
...
def process_master_and_details(client_wrapper: SheetsWrapper, scoring_config: Dict = None, run_followups: bool = False):
    ...
    for idx, row in detail_df.iterrows():
        ...
        # Enrich
        # Pass raw resume text so score_candidate can re-run detect_education on it
        candidate['education_text'] = candidate.get('resume_text', '')
...
def main():
    ...
    if args.schedule:
        ...
        sched = get_scheduler()
        sched.add_job(lambda: process_master_and_details(wrapper, scoring_config=DEFAULT_SCORING, run_followups=True), 'interval', hours=12, id='daily-run')
        sched.start()
